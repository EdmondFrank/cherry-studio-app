...
> {
  ...
  public buildSdkMessages(
    ...
    const assistantMessage: OpenAISdkMessageParam = {
      ...
      reasoning_content: toolCalls.length > 0 ? '' : undefined
    }
    ...
    return {
      transform: async (
        ...
        const commonParams: OpenAISdkParams = {
          ...
          ...extra_body,
          // 只在对话和总结场景下应用自定义参数，避免影响翻译等其他业务逻辑
          // 注意：用户自定义参数总是应该覆盖其他参数
          ...(coreRequest.callType === 'chat' || coreRequest.callType === 'summary' ? this.getCustomParameters(assistant) : {})
        }
